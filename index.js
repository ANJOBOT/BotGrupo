	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao *${mdata.subject}*` *Apresente-se ao Grupo*
				...
				teks = `Saiu pq a mulher pegou o Celular🤭 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '[❗] Este comando só pode ser usado por administradores de grupo, seu membro comum 😂! ❌',
			...
			const ownerNumber = ["5598989016015"] // replace this with your number
			...
			switch(command) {
				...
				case '.':
					...
					client.sendMessage(from, hisil, image, {quoted: mek, caption: (prefix), text})
					break
			    case 'help':
				case 'menu':
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `Dono do grupo e o : @${from.split("-")[0]}`,
	    ...
	    case 'kapankah':
			...
			case 'closegc':
               ...
               if (!isGroup) return reply(mess.only.group)
                ...
                if (args[0] === '1') {
                    if (isLevelingOn) return reply('*O recurso de level ja esta ativado*')
            ...
            break
					...
					case 'jokerlogo':
                            ...
                            var gh = body.slice(7)
                            var teks1 = gh.split("|")[0];
                            if (args.length < 1) return reply(`Digite Assim\nExemplo: ${prefix}coffee Toin`)
                            reply(mess.wait)
							...
							case 'jadwaltvnow':  
							   ...
							   reply(anu.result.jadwalTV)
								break
											case 'afk':
													tels = body.slice(4)
													if (args.length < 1) return reply('por que?')
													var nom = mek.participant
													const tag = {
															text: `@${nom.split("@s.whatsapp.net")[0]} *TO AFK ${tels} NAO PERTURBE*`,
											...
											case 'undergocean':
													var gh = body.slice(12)
													var teks1 = gh.split("|")[0];
													if (args.length < 1) return reply(`Digite Assim\nExemplo: ${prefix}undergocean Toin`)
													reply(mess.wait)
											...
											case 'woodenboards':
													...
													var teks1 = gh.split("|")[0];
													if (args.length < 1) return reply(`Digite Assim\nExemplo: ${prefix}woodenboards Toin`)
													reply(mess.wait)
											...
											case 'metalictglow':
													...
													var teks1 = gh.split("|")[0];
													if (args.length < 1) return reply(`Digite Assim\nExemplo: ${prefix}metalictglow Toin`)
													reply(mess.wait)
