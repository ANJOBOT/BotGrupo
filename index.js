...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\ Seja bem vindo ao grupo *${mdata.subject}* Por Favor não seja um ghost♥️`
				...
				teks = `Saiu do grupo👉 @${num.split('@')[0]} E Ninguém se importa🤭🎊`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           case 'rebaixar':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Removido Com Sucesso palhaço 🤡  :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
