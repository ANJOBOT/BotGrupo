...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\ Olá *${mdata.subject}* Bem vindo ao Grupo, Por Favor Não Seja Ghost♥️`
				...
				teks = `Saiu Do Grupo @${num.split('@')[0]} Nem volte mais😒`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
       ...
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Perdeu Adm @${mentioned[0].split('@')[0]} seu mula`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
				case 'promover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Parabens agr vc e ADM :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabens Agr Vc e ADM @${mentioned[0].split('@')[0]} esse implorou pra ser ADM kkkk`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
			...
			case 'remover':
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *Ativar/Desativar?') 
